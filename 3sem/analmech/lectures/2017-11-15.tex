...
\subsubsection{Геометрическая интерпретация Мак-Гуллока}
\begin{flalign*}
& Ap^2 + Bq^2 + Cr^2 = 2h &\\
& A^2p^2 + B^2q^2 + C^2r^2 = k^2 &\\
& k_\xi = Ap, \quad k_\eta = Bq, \quad k_\zeta = Cr &\\
& S = \eta \v k:\;\; k_\xi^2 + k_\eta^2 + k_\zeta^2 = k^2 &\\
& \Phi = \left\{ \v k:  \frac{k_\xi^2}{A} + \frac{k_\eta^2}{B} + \frac{k_\zeta^2}{C} = 2h \right\} \text{ --- эллипсоид Мак-Гуллока} &\\
\end{flalign*}
...
& = \v e_{x'} \dot \psi \dot \varphi \sin \Theta \cdot C \left( 1 + \frac{C - A}{C} \frac{\dot \psi}{\dot \varphi} \right) \Rightarrow &\\
\end{flalign*}
\[
